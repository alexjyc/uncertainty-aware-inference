...
def eval_multiple_choice(model, tokenizer, examples: list[dict], temperature: float = 1.0):
    ...
    for i, ex in enumerate(examples):
        ...
        for choice in ex["choices"]:
            ...
            with torch.no_grad():
                logits = model(input_ids).logits.to(torch.float32) / temperature
...
def eval_generative_qa(model, tokenizer, examples: list[dict], temperature: float = 1.0):
    """Evaluate via greedy decode with scaled confidence and entropy."""
    all_confidences = []
    all_accuracies = []
    all_entropies = []
    total_tokens = 0
    ...
    for i, ex in enumerate(examples):
        ...
        for step, token_id in enumerate(generated_ids):
            step_logits = outputs.scores[step][0] / temperature
...
def plot_confidence_distribution(confidences: torch.Tensor, accuracies: torch.Tensor, save_path: str,
    title: str = "Confidence Distribution", num_bins: int = 20) -> None:
    """Create and save a histogram of confidence scores for correct vs incorrect predictions."""
    save_path = Path(save_path)
    save_path.parent.mkdir(parents=True, exist_ok=True)
    
    c = confidences.cpu().numpy()
    a = accuracies.cpu().numpy()
    ...
    if len(correct_c) > 0:
        ax.hist(correct_c, bins=bins, alpha=0.6, color='green', label='Correct', density=True)
    if len(incorrect_c) > 0:
        ax.hist(incorrect_c, bins=bins, alpha=0.6, color='red', label='Incorrect', density=True)
...
def run_eval(
    ...
    for ds_name in datasets_to_run:
        print(f"\n{'='*50}\nEvaluating: {ds_name} (Temp: {temperature})\n{'='*50}")

        cfg = DATASET_CONFIGS[ds_name]
        examples = load_eval_dataset(ds_name, max_samples, seed)
        print(f"Loaded {len(examples)} examples")

        if cfg["type"] == "multiple_choice":
            confidences, accuracies, entropies, tps = eval_multiple_choice(
                model, tokenizer, examples, temperature
            )
        else:
        ...
        plot_confidence_distribution(
            confidences, accuracies,
            save_path=f"{prefix}_confidence.png",
            title=f"Confidence: {model_tag} {quant_method.upper()} {precision} - {ds_name}",
        )
